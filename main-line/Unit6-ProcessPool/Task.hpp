...
std::unordered_map<int, std::string> desc; // 定义这些函数的描述，比如0代表readMySQL，里面的string就是“访问数据库”

void readMySQL()
{
    std::cout << "process[" << getpid() << "] 访问数据库的任务" << std::endl
              << std::endl;
}
void execuleUrl()
{
...
// 加载任务，这些任务都是需要父进程获取，然后交给子进程去执行的
void load()
{
